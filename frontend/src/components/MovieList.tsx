...
export default function MovieList({ movies }: MovieListProps) {
  ...
  const [sortField, setSortField] = useState<SortField>('physicalReleaseDate');
  ...
  const filteredAndSortedMovies = useMemo(() => {
    return movies
      ...
      .sort((a, b) => {
        ...
        return sortOrder === 'asc'
          ? new Date(a.physicalReleaseDate).getTime() - new Date(b.physicalReleaseDate).getTime()
          : new Date(b.physicalReleaseDate).getTime() - new Date(a.physicalReleaseDate).getTime();
      });
        ...
        <table className="min-w-full">
          <thead>
            <tr className="border-b border-gray-800">
              ...
              <th 
                className="px-6 py-3 text-left text-xs font-medium text-gray-400 cursor-pointer hover:text-white"
                onClick={() => handleSort('physicalReleaseDate')}
              >
                Release Date <SortIcon field="physicalReleaseDate" />
              </th>
            </tr>
          </thead>
          <tbody>
