...
export function parseTorrentFilename(filename: string): ParsedTorrent {
    ...
    const title_regex =
        /^(.*?)((19|20)\d{2}|S\d{2}|E\d{2}|x\d{2}|720p|1080p|2160p|BluRay|WEB|HDR|DVDrip|HDTV)/i;
    const title_match = title_regex.exec(fname);
    result.title = title_match
        ? title_match[1].replace(/[._]/g, " ").trim()
        : undefined;

    result.resolution = matchSingle(
        fname,
        REGEXES.resolutions
    ) as ParsedTorrent["resolution"];
    result.quality = matchSingle(
        fname,
        REGEXES.qualities
    ) as ParsedTorrent["quality"];
    result.encode = matchSingle(
        fname,
        REGEXES.encodes
    ) as ParsedTorrent["encode"];
    ...
    result.visualTags = matchMultiple(
        fname,
        REGEXES.visualTags
    ) as ParsedTorrent["visualTags"];
